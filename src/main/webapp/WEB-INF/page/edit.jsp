...
<head>
    ...
    <script language="javascript" src="<%=request.getContextPath() %>/assets/js/jquery.1.9.1.min.js"></script>
    <script>
        $(document).ready(function () {
            var date = new Date();
            var str = new Array(date,date);
            $("#rangeTime").val(str);
            alert($("#rangeTime").val());
        });
    </script>
...
<center>
    ...
    <div>
        <form:form method="post" action="update" modelAttribute="user">
            <input type="hidden" id="test" name="startDate"/>
            <input type="hidden" id="rangeTime" name="rangeTime"/>
            <table>
                <tr>
                    <td>First Name :</td>
